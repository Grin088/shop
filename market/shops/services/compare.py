...
class CompareMixin:
    def get(self, request: HttpRequest) -> HttpResponse:
        ...
        if comp_list and len(comp_list) > 1:
            ...
            ) = splitting_into_groups_by_category(comp_list)
            list_compare, list_property = comparison_lists_and_properties(
                list(category_offer_dict.values())[0]
            )
            context = {
        ...
        return render(
            request,
            "market/shops/comparison.jinja2",
    ...
    def post(self, request) -> HttpResponse:
        ...
        if len(comp_list) > 1:
            ...
            list_compare, list_property = comparison_lists_and_properties(
                category_offer_dict[category_name]
            )
            context = {
                "category_offer_dict": category_count_product,
                "list_compare": list_compare,
...
def splitting_into_groups_by_category(
    comp_list: list[int],
) -> (dict[str, list[int]], list[(str, int)]):
    """Разбивание списка сравнения по категориям товара"""

    category_offer_dict = {}
    offer_category = Offer.objects.filter(pk__in=comp_list).values(
        "product__category_id__name", "id"
    )
    ...
    category_count_product = sorted(
        [(name, len(count)) for name, count in category_offer_dict.items()],
        key=lambda x: x[1],
        reverse=True,
    )
...
def _get_a_complete_list_of_property_names(list_offer: list[int]) -> list[str]:
    ...
    list_name_property = list(
        set(
            Offer.objects.filter(pk__in=list_offer).values_list(
                "product__property__name", flat=True
            )
        )
    )
...
def _adding_missing_properties(
    list_property: list[str], list_compare: ListCompare
) -> ListCompare:
...
def _comparison_of_product_properties(
    list_compare: ListCompare, list_property: list[str]
) -> ListCompare:
